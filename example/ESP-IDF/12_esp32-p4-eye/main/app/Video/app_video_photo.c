...
static const uint32_t photo_resolution_width[PHOTO_RESOLUTION_MAX] = {640, 1280, 1280, 1920};
static const uint32_t photo_resolution_height[PHOTO_RESOLUTION_MAX] = {480, 720, 960, 1080};
...
static void app_video_calc_center_crop(uint32_t in_width, uint32_t in_height, uint32_t target_width, uint32_t target_height,
                                       uint32_t *crop_width, uint32_t *crop_height)
{
    if (crop_width == NULL || crop_height == NULL || in_width == 0 || in_height == 0) {
        return;
    }
    ...
    uint64_t in_ratio = (uint64_t)in_width * target_height;
    uint64_t target_ratio = (uint64_t)in_height * target_width;

    if (in_ratio > target_ratio) {
        *crop_height = in_height;
        *crop_width = (uint32_t)((uint64_t)in_height * target_width / target_height);
    } else {
        *crop_width = in_width;
        *crop_height = (uint32_t)((uint64_t)in_width * target_height / target_width);
    }
...
esp_err_t take_and_save_photo(uint8_t *camera_buf, uint32_t width, uint32_t height)
{
    ...
    if(current_resolution != PHOTO_RESOLUTION_1080P) {
        ...
        // Check if resolution exceeds shared buffer capacity (1280x960 max)
        if (photo_width > SHARED_PHOTO_BUF_WIDTH || photo_height > SHARED_PHOTO_BUF_HEIGHT) {
            ...
            photo_width = (photo_width > SHARED_PHOTO_BUF_WIDTH) ? SHARED_PHOTO_BUF_WIDTH : photo_width;
            photo_height = (photo_height > SHARED_PHOTO_BUF_HEIGHT) ? SHARED_PHOTO_BUF_HEIGHT : photo_height;
        }

        uint32_t crop_width = 0;
        uint32_t crop_height = 0;
        app_video_calc_center_crop(width, height, photo_width, photo_height, &crop_width, &crop_height);
    ...
    if (ret != ESP_OK) {
        ...
        bsp_display_lock(0);
        ui_extra_popup_take_photos_count();
        bsp_display_unlock();
    }
