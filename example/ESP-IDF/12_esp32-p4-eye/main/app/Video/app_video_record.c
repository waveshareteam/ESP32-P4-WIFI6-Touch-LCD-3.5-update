...
esp_err_t take_and_save_video(uint8_t *camera_buf, uint32_t width, uint32_t height)
{
    ...
    uint32_t photo_width = VIDEO_RECORD_WIDTH;
    uint32_t photo_height = VIDEO_RECORD_HEIGHT;
...
esp_err_t app_video_record_init(void)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to initialize audio codec: 0x%x", ret);
    }
...
static void audio_capture_encode_task(void *pvParameters)
{
    ...
    while (recorder_ctx.recording) {
        ...
        size_t bytes_read = 0;
        ret = bsp_extra_i2s_read(sample_buffer, pcm_frame_size, &bytes_read, 100);
