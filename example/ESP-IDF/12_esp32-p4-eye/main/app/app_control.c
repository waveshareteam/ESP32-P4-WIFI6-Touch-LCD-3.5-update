...
static void btn_handler(void *arg, void *data)
{
    ...
    switch (button_id) {
        case BSP_BUTTON_1:
            ui_extra_key_input(UI_EXTRA_KEY_MENU);
            break;
            
        case BSP_BUTTON_2:
            ui_extra_key_input(UI_EXTRA_KEY_UP);
            break;
            
        case BSP_BUTTON_3:
            ui_extra_key_input(UI_EXTRA_KEY_DOWN);
            break;
            
        case BSP_BUTTON_ED:
            ui_extra_key_input(UI_EXTRA_KEY_OK);
            break;
...
static const char *TAG = "app_control";
static button_handle_t single_btn;

static void single_btn_handler(void *button_handle, void *usr_data)
{
    ui_extra_key_t key = (ui_extra_key_t)(uintptr_t)usr_data;
...
esp_err_t app_control_init(void)
{
    ...
    if (single_btn == NULL) {
        esp_err_t ret = iot_button_new_gpio_device(&btn_cfg, &btn_gpio_cfg, &single_btn);
        if (ret != ESP_OK || single_btn == NULL) {
            ESP_LOGE(TAG, "gpio35 button init failed: %d", ret);
            return ret;
        }
        ...
        ESP_ERROR_CHECK(iot_button_register_cb(single_btn, BUTTON_SINGLE_CLICK, NULL, single_btn_handler, (void *)(uintptr_t)UI_EXTRA_KEY_OK));
        ESP_ERROR_CHECK(iot_button_register_cb(single_btn, BUTTON_DOUBLE_CLICK, NULL, single_btn_handler, (void *)(uintptr_t)UI_EXTRA_KEY_DOWN));
        ESP_ERROR_CHECK(iot_button_register_cb(single_btn, BUTTON_MULTIPLE_CLICK, &triple_args, single_btn_handler, (void *)(uintptr_t)UI_EXTRA_KEY_UP));
        ESP_ERROR_CHECK(iot_button_register_cb(single_btn, BUTTON_LONG_PRESS_START, NULL, single_btn_handler, (void *)(uintptr_t)UI_EXTRA_KEY_MENU));
    }
