...
esp_err_t app_storage_save_settings(settings_info_t *settings, uint16_t interval_time, uint16_t magnification)
{
    ...
    if (strcmp(settings->resolution, "1080P") == 0) {
        ...
        resolution_idx = 2;
    } else if (strcmp(settings->resolution, "960P") == 0) {
        resolution_idx = 3;
    }
    ...
    while(1) {
        ...
        if (current_is_sd_card_present != is_sd_card_present) {
            ...
            if (is_sd_card_present) {
                ...
                if (ret == ESP_OK) {
                    DIR *mount_dir = opendir(BSP_SD_MOUNT_POINT);
                    if (mount_dir != NULL) {
                        closedir(mount_dir);
                        is_sd_card_mounted = true;
                        ESP_LOGI(TAG, "SD card mounted: %s", BSP_SD_MOUNT_POINT);
                    } else {
                        is_sd_card_mounted = false;
                        bsp_sdcard_unmount();
                        ESP_LOGE(TAG, "SD card mount verify failed: %s", BSP_SD_MOUNT_POINT);
                    }
                } else {
                    is_sd_card_mounted = false;
                ...
                bsp_display_lock(0);
                ui_extra_set_sd_card_mounted(is_sd_card_mounted);
                bsp_display_unlock();
