...
esp_err_t bsp_touch_new(const bsp_touch_config_t *config, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    bsp_touch_config_t default_config = {0};
    if (config == NULL) {
...
static lv_indev_t *bsp_display_indev_init(lv_display_t *disp, const bsp_display_cfg_t *cfg)
{
    BSP_ERROR_CHECK_RETURN_NULL(bsp_touch_new(&cfg->touch_flags, &tp));
    assert(tp);
...
lv_display_t *bsp_display_start_with_config(bsp_display_cfg_t *cfg)
{
    ...
    BSP_NULL_CHECK(disp_indev = bsp_display_indev_init(disp, cfg), NULL);
